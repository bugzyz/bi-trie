...
int cal_common_prefix_len(const char* s1, int sz1, const char* s2, int sz2) {
    if (sz1 > sz2) {
        sz1 = sz2;
    }
    for (int i = 0; i != sz1; i++) {
   ...
   public:
    ...
    class hash_node : public anode {
       public:
        ...
        map<CharT, triple> element_num_of_1st_char;

        class triple {
           public:
            uint16_t char_elem_num;
            char* common_prefix;
            uint16_t common_prefix_len;

            string get_prefix() {
                return string(common_prefix, common_prefix_len);
            }

            triple()
                : char_elem_num(0),
                  common_prefix(nullptr),
                  common_prefix_len(UINT16_MAX) {}

            void cal_new_key(char* key, size_t keysize) {
                // add first elem with this first char
                if (common_prefix == nullptr) {
                    char_elem_num++;
                    common_prefix = key;
                    common_prefix_len = keysize;
                    return;
                }
                // update second elem with this first char
                char_elem_num++;
       ...
       public:
        ...
        void burst(trie_node* p, htrie_map* hm, std::string prefix) {
            ...
            if (element_num_of_1st_char.size() == 1) {
                ...
                auto it = element_num_of_1st_char.begin();
                char c = it->first;
                triple only_triple = it->second;

                global_common_prefix_len = it->second.common_prefix_len;

                // create the chain with several single trie_node
                // the number of node is common_prefix_len
                for (int i = 0; i != global_common_prefix_len; i++) {
                    if (p == nullptr) {
                ...
                for (int i = 0; i != Bucket_num; i++) {
                    for (int j = 0; j != Associativity; j++) {
                        ...
                        element_num_of_1st_char[key[global_common_prefix_len]]
                            .cal_new_key(key, s->length);
                    }
                }
            ...
            for (auto it = element_num_of_1st_char.begin();
                ...
                if (p == nullptr) {
                    // bursting in a root hashnode
                    // the t_root is updated to a empty trie_node
                    p = new trie_node(nullptr);
                    hm->setRoot(p);
                }
                ...
                size_t expected_associativity =
                    (double)it->second.char_elem_num / (double)Bucket_num /
                        Burst_ratio +
                    1;
                ...
                hash_node* hnode =
                    new hash_node(local_parent,
                                  prefix + string(local_common_prefix,
                                                  local_common_prefix_len),
                                  expected_associativity);
                local_parent->set_hash_node_child(hnode);
            ...
            for (int i = 0; i != Bucket_num; i++) {
                for (int j = 0; j != Associativity; j++) {
                    ...
                    CharT first_char = key[global_common_prefix_len];

                    size_t local_common_prefix_len =
                        element_num_of_1st_char[first_char].common_prefix_len;

                    hash_node* hnode = hnode_set[first_char];
                    ...
                    if (length_left == 0) {
                        ...
                        string new_prefix =
                            prefix +
                            element_num_of_1st_char[first_char].get_prefix();
            ...
            for (auto it = burst_again_list.begin();
                ...
                // burst
                string burst_again_prefix =
                    prefix + element_num_of_1st_char[it->first].get_prefix();
        ...
        std::pair<bool, T> insert_kv_in_hashnode(const CharT* key,
            ...
            // debug
            if (!(slotid != -1 && slotid >= 0 && slotid < cur_associativity))
                print_key_metas();
