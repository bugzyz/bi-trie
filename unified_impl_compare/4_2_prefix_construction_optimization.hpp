   ...
   public:
    ...
    class trie_node : public anode {
       public:
        ...
        // a CharT for constructing prefix
        CharT myChar;

        trie_node(trie_node* p, CharT c)
            : have_value(false),
              value(T()),
        ...
        trie_node* find_trie_node_child(CharT c, const CharT* key,
            ...
            if (found != trie_node_childs.end()) {
                ...
                trie_node* son_trie_node = new trie_node(this, c);
       ...
       public:
        ...
        void burst(trie_node* p, htrie_map* hm, std::string prefix) {
            ...
            if (common_prefix_len != 0) {
                ...
                for (int i = 0; i != common_prefix_len; i++) {
                    if (p == nullptr) {
                        ...
                        p = new trie_node(nullptr, '\0');
                        hm->setRoot(p);
                    }

                    trie_node* cur_trie_node = new trie_node(p, first_key_p[i]);
            ...
            for (auto it = element_num_of_1st_char.begin();
                ...
                trie_node* cur_trie_node = new trie_node(p, it->first);
    ...
    class SearchPoint {
       public:
        ...
        std::string get_string() {
            ...
            if (node->is_hash_node()) {
                cur_node = node->anode::parent;
            } else {
                cur_node = (trie_node*)node;
            }
            std::string res;
            ...
            if (index != -1) {
                ...
                res = res + string(((hash_node*)node)->get_tail_pointer(sl),
                                   sl->length);
            }
            return res;
        }
